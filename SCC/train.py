...
def train():
    loss_epoch = 0
    # for step, ((x_i, x_j), labels) in enumerate(data_loader):
    for step, ((x_i, x_j), labels, img, x_center, y_center, W, H) in enumerate(data_loader):
        optimizer.zero_grad()
        x_i = x_i.to('cuda')
        x_j = x_j.to('cuda')
        z_i, z_j, c_i, c_j = model(x_i, x_j)
        loss_instance = criterion_instance(z_i, z_j, labels)
...
if __name__ == "__main__":
    ...
    writer = SummaryWriter('./save/log/'+ args.model_path[5:])
    # prepare data
    if args.dataset == "CIFAR-10":
        ...
        dataset = torchvision.datasets.ImageFolder(
            root='/media/F/sub_voc/train',
            # root='/mnt/nfs/data/home/1120220291/lwy/cc/datasets/voc_coco/train',
            transform=transform.Transforms(s=0.5, size=args.image_size),
        )
        class_num = 80
    elif args.dataset == "mcc":
        dataset = CustomDataset(
            './datasets/t1_train_all',
                    transform=transform.Transforms(s=0.5, size=args.image_size),
        )
        class_num = args.cats
    else:
