...
class TrainerBase:
        ...
        with EventStorage(start_iter) as self.storage:
            try:
                ...
                for self.iter in range(start_iter, max_iter):
                    if self.cfg.OWDet.SKIP_TRAINING_WHILE_EVAL:
                        continue
                    self.before_step()
                    self.run_step()
                    self.after_step()
    ...
    def before_train(self):
        if self.cfg.OWDet.SKIP_TRAINING_WHILE_EVAL:
            logger = logging.getLogger(__name__)
            logger.info('Skipping training as cfg.OWDet.SKIP_TRAINING_WHILE_EVAL flag is set.')
        for h in self._hooks:
            h.before_train()

    def after_train(self):
        self.storage.iter = self.iter
        if self.cfg.OWDet.COMPUTE_ENERGY:
            logger = logging.getLogger(__name__)
    ...
    def analyse_energy(self, temp=1.5):
        files = os.listdir(os.path.join(self.cfg.OUTPUT_DIR, self.cfg.OWDet.ENERGY_SAVE_PATH))
        temp = self.cfg.OWDet.TEMPERATURE
        logger = logging.getLogger(__name__)
        ...
        for id, file in enumerate(files):
            path = os.path.join(self.cfg.OUTPUT_DIR, self.cfg.OWDet.ENERGY_SAVE_PATH, file)
            try:
                logits, classes = torch.load(path)
            except:
                logger.info('Not able to load ' + path + ". Continuing...")
                continue
            num_seen_classes = self.cfg.OWDet.PREV_INTRODUCED_CLS + self.cfg.OWDet.CUR_INTRODUCED_CLS
        ...
        param_save_location = os.path.join(self.cfg.OUTPUT_DIR,
                                           'energy_dist_' + str(self.cfg.OWDet.PREV_INTRODUCED_CLS
                                                                + self.cfg.OWDet.CUR_INTRODUCED_CLS) + '.pkl')
