...
class PascalVOCDetectionEvaluator(DatasetEvaluator):
    ...
    def __init__(self, dataset_name, cfg=None):
        ...
        if cfg is not None:
            self.prev_intro_cls = cfg.OWDet.PREV_INTRODUCED_CLS
            self.curr_intro_cls = cfg.OWDet.CUR_INTRODUCED_CLS
    ...
    def evaluate(self):
        ...
        with tempfile.TemporaryDirectory(prefix="pascal_voc_eval_") as dirname:
            ...
            for cls_id, cls_name in enumerate(self._class_names):
                ...
                with open(res_file_template.format(cls_name), "w") as f:
                    f.write("\n".join(lines))
                    # add-huatu
                # with open(os.path.join("./output/t3_train", "{}.txt").format(cls_name) , "w") as f:
                #     f.write("\n".join(lines))

                # for thresh in range(50, 100, 5):
                thresh = 50
                rec, prec, ap, unk_det_as_known, num_unk, tp_plus_fp_closed_set, fp_open_set, tp_forcc = voc_eval(
                    res_file_template,
        ...
        self._logger.info('UDR ===  ' + str((total_num_unk_det_as_known[thresh]+np.sum(tp_forcclist))/total_num_unk))
        self._logger.info('UDP === ' + str((np.sum(tp_forcclist))/(total_num_unk_det_as_known[thresh]+np.sum(tp_forcclist))))
...
def voc_eval(detpath, annopath, imagesetfile, classname, ovthresh=0.5, use_07_metric=False, known_classes=None):
    ...
    with PathManager.open(imagesetfile, "r") as f:
        lines = f.readlines()
    imagenames = set([x.strip() for x in lines])
    ...
    for imagename in imagenames:
        ...
        real_name = [x["real name"] for x in R]
        difficult = np.array([x["difficult"] for x in R]).astype(bool)
        # difficult = np.array([False for x in R]).astype(bool)  # treat all "difficult" as GT
        det = [False] * len(R)
        npos = npos + sum(~difficult)
        class_recs[imagename] = {"bbox": bbox, "difficult": difficult, "det": det, "real name":real_name}
    ...
    for d in range(nd):
        ...
        if BBGT.size > 0:
            ...
            jmax = np.argmax(overlaps)
                ...
                # add-huatu
        if classname == "unknown" and ovmax > ovthresh:
            with open("./output/t1_train_unknown_class/"+R["real name"][jmax]+".txt","a") as f :
                line = " ".join([image_ids[d],str(confidence[d]),str(bb[0]),str(bb[1]),str(bb[2]),str(bb[3])])
                f.write(line)
                f.write("\n")
    ...
    for imagename in imagenames:
        ...
        bbox = np.array([x["bbox"] for x in R])
        # difficult = np.array([x["difficult"] for x in R]).astype(bool)
        difficult = np.zeros(len(R)).astype(bool)
        det = [False] * len(R)
        n_unk = n_unk + sum(~difficult)
        unknown_class_recs[imagename] = {"bbox": bbox,  "det": det}

    if classname == 'unknown':
        return rec, prec, ap, 0, n_unk, None, None, 0 if len(tp)==0 else tp[-1]
    ...
    for d in range(nd):
        ...
        if BBGT.size > 0:
            ...
            jmax = np.argmax(overlaps)
                ...
                # add-huatu
        # if classname == "unknown" and ovmax > ovthresh:
        #     with open("./output/t3_train_unknown_class/"+R["real name"][jmax]+".txt","a") as f :
        #         line = " ".join([image_ids[d],str(confidence[d]),str(bb[0]),str(bb[1]),str(bb[2]),str(bb[3])])
        #         f.write(line)
        #         f.write("\n")
    ...
    return rec, prec, ap, is_unk_sum, n_unk, tp_plus_fp_closed_set, fp_open_set, 0


def plot_pr_curve(precision, recall, filename, base_path='/home/fk1/workspace/OWDet/output/plots/'):
    fig, ax = plt.subplots()
